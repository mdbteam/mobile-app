...
const axiosInstance = axios.create({
  baseURL: 'https://auth-service-1-8301.onrender.com/api',
});

const fetchPrestador = async (id_prestador: string) => {
  const { data } = await axiosInstance.get<PrestadorDetalle>(`/prestadores/${id_prestador}`);
  return data;
};

export default function PrestadorDetailPage() {
  const params = useLocalSearchParams();
  const id_prestador = params.id as string;

  const { data: prestador, isLoading, error } = useQuery({
    queryKey: ['prestador', id_prestador],
    queryFn: () => fetchPrestador(id_prestador),
  });
